...
from fastgs.surface.utils.loss_utils import l1_loss, ssim, lncc, get_img_grad_weight
from fused_ssim import fused_ssim as fast_ssim
from fastgs.surface.utils.graphics_utils import patch_offsets, patch_warp
from fastgs.surface.gaussian_renderer import render, network_gui
import sys, time
from fastgs.surface.scene import Scene, GaussianModel
import cv2
import uuid
from tqdm import tqdm
from fastgs.surface.utils.image_utils import psnr, erode
from argparse import Namespace
from fastgs.surface.scene.app_model import AppModel
from fastgs.surface.scene.cameras import Camera
try:
...
from fastgs.surface.utils.fast_utils import compute_gaussian_score_fastgs, sampling_cameras
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # backup the package source code used for this run (for reproducibility)
    try:
        pkg_dir = os.path.dirname(os.path.abspath(__file__))
        shutil.copytree(pkg_dir, os.path.join(dataset.model_path, "source_backup"),
                        dirs_exist_ok=True,
                        ignore=shutil.ignore_patterns("__pycache__", "*.pyc"))
    except Exception as e:
        print(f"[warning] source backup failed: {e}")
